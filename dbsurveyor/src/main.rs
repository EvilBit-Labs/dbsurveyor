...
pub fn validate_cli_paths(cli: &Cli) -> Result<Cli, PathValidationError> {
    ...
    let base_dir = std::env::current_dir()
        .map_err(|e| PathValidationError::CurrentDirFailed { source: e })?;

    match &cli.command {
        Some(Commands::Process { input, format, output }) => {
            // Validate input path
...
mod tests {
    ...
    fn test_validate_file_path_success() {
        ...
        let canonical_base_dir = base_dir.canonicalize().unwrap_or_else(|_| base_dir.to_path_buf());
    ...
    fn test_validate_file_path_directory_traversal() {
        ...
        match result.unwrap_err() {
            PathValidationError::DirectoryTraversal { path } => {
    ...
    fn test_validate_file_path_leading_separator_relative() {
        ...
        assert!(result.is_err());
        match result.unwrap_err() {
            PathValidationError::OutsideBaseDirectory { path } => {
    ...
    fn test_validate_cli_paths_success() {
        ...
        if let Some(Commands::Process { input, output, .. }) = validated_cli.command {
            ...
            assert!(output.unwrap().starts_with(&canonical_base_dir));
        }
