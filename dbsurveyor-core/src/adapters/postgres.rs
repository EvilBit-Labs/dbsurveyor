...
impl PostgresAdapter {
    ...
    ///
    /// # Arguments
    /// * `action_rule` - PostgreSQL referential action rule (either single char code or full name)
    ///
    /// # Returns
    /// Returns the corresponding ReferentialAction or None if unknown
    ///
    /// # PostgreSQL Referential Actions
    /// - CASCADE (c): Automatically delete/update dependent rows
    /// - SET NULL (n): Set foreign key columns to NULL
    /// - SET DEFAULT (d): Set foreign key columns to their default values
    /// - RESTRICT (r): Prevent delete/update if dependent rows exist
    /// - NO ACTION (a): Same as RESTRICT but check can be deferred
    pub fn map_referential_action(action_rule: &str) -> Option<crate::models::ReferentialAction> {
        ...
        match action_rule.to_uppercase().as_str() {
            // Full action names (from information_schema.referential_constraints)
            ...
            // Single character codes (from pg_constraint system catalog)
